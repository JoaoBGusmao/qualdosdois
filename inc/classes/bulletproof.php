...
class Image implements \ArrayAccess
{
     ...
     * @return $this
     */
    public function setLocation($dir = "../upload", $permission = 0666)
    {
    ...
    public function upload()
    {
        ...
        if (!in_array($image->mime, $image->mimeTypes)) {
            ...
            $image->error = "Arquivo inválido. São aceitos somente arquivos nas extensões ($ext) ";
            return ;
        }     
        ...
        if ($image->height > $allowedHeight || $image->width > $allowedWidth) {
            $image->error = "A altura x largura da imagem deve ser menor do que ' $allowedHeight \ $allowedWidth ' pixels";
            return ;
        }

        if($image->height < 4 || $image->width < 4){
            $image->error = "Imagem muito pequena ou corrompida"; 
            return ;
        }
        ...
        $image->error = "Upload falhou. Erro desconhecido";
        return false;
    }
